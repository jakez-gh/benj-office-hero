...
def test_patch_adapter_unknown_adapter_422(client):
    resp = client.patch(f"/admin/tenants/{uuid4()}/adapter", json={"adapter": "unknown_crm"})
    assert resp.status_code == 422


def test_patch_adapter_no_db_returns_503(client):
    resp = client.patch(f"/admin/tenants/{uuid4()}/adapter", json={"adapter": "jobber"})
    assert resp.status_code == 503


def test_patch_adapter_requires_operator(unauthed_client):
    resp = unauthed_client.patch(f"/admin/tenants/{uuid4()}/adapter", json={"adapter": "jobber"})
    assert resp.status_code == 403
...
def test_patch_adapter_all_valid_values_accepted(client):
    ...
    for adapter in ("native", "servicetitan", "pestpac", "jobber"):
        resp = client.patch(f"/admin/tenants/{uuid4()}/adapter", json={"adapter": adapter})
        assert (
            resp.status_code == 503
        ), f"Expected 503 for adapter={adapter!r}, got {resp.status_code}"
