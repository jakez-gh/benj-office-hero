...
async def test_resume_rolls_stale_next_due_forward(svc, cust_repo, loc_repo, contract_repo):
    """Resuming a long-paused contract must not dump a backlog of stale visits."""
    cust, loc = await _seed_customer_and_location(cust_repo, loc_repo)
    ...
    refreshed = await contract_repo.get_by_id(resumed.id, TENANT_A)
    assert refreshed.next_due >= datetime.now(UTC).date()  # same clock as the service
